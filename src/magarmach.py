...
class magarmach:
    def __init__(self,peer):
        ...
        self.Mine({"sender": "0x", "receiver": "0x"})

    def Block(self, index, hash, prev_hash, pow, data):
        return {
            "index": index,
    ...
    def Mine(self, data):
        ...
        if self.chain_length > 0:
            ...
            pow = self.PoW(index,prev_block["pow"], data)
            prev_hash = prev_block["hash"]

            block = self.Block(index,"",prev_hash, pow, data)
            block["hash"] = self.Hash(block);
        else:
            block = self.Block(0, "", None, 0, data)
            block["hash"] = self.Hash(block)
    ...
    def PoW(self, index, prev_pow, data):
        pow = 1
        solve = "0"*self.difficulty

        while True:
            digest = self.toDigest(index,pow,prev_pow,data)
    ...
    def toDigest(self,index,pow,prev_pow,data):
        return (str((pow+prev_pow+random.randint(99,999999)**2)+index)+json.dumps(data)).encode()
